...
All metrics use the `kafka_consumer_` namespace prefix and are registered via the `prometheus.Registerer` passed to `consumer.WithMetrics()`.
...
| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `kafka_consumer_messages_polled_total` | Counter | `partition` | Total messages received from Kafka, per partition. Primary ingestion throughput signal. |
| `kafka_consumer_poll_errors_total` | Counter | — | Total `Poll()` errors. |
| `kafka_consumer_commits_total` | Counter | `partition` | Total successful offset commits, per partition. |
| `kafka_consumer_commit_failures_total` | Counter | — | Total failed offset commits. Bulk commits succeed or fail atomically, so partition attribution is not meaningful. Consecutive failures trigger degraded mode. |
| `kafka_consumer_degraded_mode` | Gauge | — | Whether the poll loop is in degraded mode (1=degraded, 0=normal). |
| `kafka_consumer_partitions_assigned` | Gauge | — | Number of partitions currently assigned to this consumer instance. Oscillation signals rebalance storms. |
| `kafka_consumer_rebalances_total` | Counter | — | Total rebalance events (incremented on each revoke). Rate > 2/hour signals consumer group instability. |
| `kafka_consumer_last_committed_offset` | Gauge | `partition` | Last successfully committed offset per partition. A flat value while `messages_polled_total` grows indicates a stuck partition. |
| `kafka_consumer_partitions_paused` | Gauge | — | Number of partitions currently paused due to backpressure or degraded mode. Direct flow-control visibility. |

## Dispatcher Metrics

| Metric | Type | Labels | Description |
|--------|------|--------|-------------|
| `kafka_consumer_messages_processed_total` | Counter | `status`, `partition` | Total messages processed. Status: `success`, `non_retryable`, `retries_exhausted`. |
| `kafka_consumer_message_delay_seconds` | Histogram | `partition` | Time from poll to processing completion per message (`time.Since(msg.PolledAt)`). Captures queueing, batching, channel wait, and processor execution time. Operator SLO metric. |
| `kafka_consumer_processing_time_seconds` | Histogram | `partition` | Processor function execution time only (`time.Since(start)` around `processor(ctx, batch)`). Developer tuning metric — isolates target service latency from pipeline overhead. |
| `kafka_consumer_record_age_seconds` | Histogram | `partition` | End-to-end latency from message production to processing completion (`time.Now() - msg.Timestamp`). Captures broker transit, partition wait, and framework delay. The time-domain equivalent of consumer lag — directly answers "how stale are the messages we're processing?" |
| `kafka_consumer_inflight_messages` | Gauge | `partition` | Messages currently buffered in worker channels or being processed, per partition. Backpressure visibility — shows which partition is filling up. |
| `kafka_consumer_circuit_breaker_state` | Gauge | — | Current circuit breaker state: 0=closed, 1=half-open, 2=open. |
| `kafka_consumer_workers_active` | Gauge | — | Number of workers currently processing a batch. |
| `kafka_consumer_dlq_messages_total` | Counter | `reason`, `partition` | Messages sent to the DLQ. Reason: `non_retryable`, `retries_exhausted`. |

## PromQL Dashboard Queries

### Ingestion throughput (messages/sec)
```promql
sum(rate(kafka_consumer_messages_polled_total[5m]))
```

### Ingestion throughput per partition
```promql
rate(kafka_consumer_messages_polled_total[5m])
```

### Processing throughput (messages/sec)
```promql
sum(rate(kafka_consumer_messages_processed_total[5m]))
```

### Ingestion vs processing gap (backlog growth rate)
```promql
sum(rate(kafka_consumer_messages_polled_total[5m]))
- sum(rate(kafka_consumer_messages_processed_total{status="success"}[5m]))
```
...
```promql
rate(kafka_consumer_messages_processed_total{status="success"}[5m])
/ on(partition) rate(kafka_consumer_messages_processed_total[5m])
```

### p99 message delay (poll-to-completion)
```promql
histogram_quantile(0.99, sum by (le)(rate(kafka_consumer_message_delay_seconds_bucket[5m])))
```

### p99 message delay per partition (spot hot partitions)
```promql
histogram_quantile(0.99, sum by (partition, le)(rate(kafka_consumer_message_delay_seconds_bucket[5m])))
```

### p99 processing time (processor function only)
```promql
histogram_quantile(0.99, sum by (le)(rate(kafka_consumer_processing_time_seconds_bucket[5m])))
```

### p99 processing time per partition
```promql
histogram_quantile(0.99, sum by (partition, le)(rate(kafka_consumer_processing_time_seconds_bucket[5m])))
```

### p99 record age (end-to-end production-to-completion)
```promql
histogram_quantile(0.99, sum by (le)(rate(kafka_consumer_record_age_seconds_bucket[5m])))
```

### p99 record age per partition (spot partitions falling behind)
```promql
histogram_quantile(0.99, sum by (partition, le)(rate(kafka_consumer_record_age_seconds_bucket[5m])))
```
...
### DLQ rate
```promql
sum(rate(kafka_consumer_dlq_messages_total[5m]))
```

### DLQ rate per partition (find problematic partitions)
```promql
rate(kafka_consumer_dlq_messages_total[5m])
```

### Circuit breaker state
```promql
kafka_consumer_circuit_breaker_state
```

### Worker utilization
```promql
kafka_consumer_workers_active
```

### Inflight messages (backpressure signal)
```promql
sum(kafka_consumer_inflight_messages)
```

### Inflight messages per partition (spot the bottleneck)
```promql
kafka_consumer_inflight_messages
```

### Commit failure rate
```promql
rate(kafka_consumer_commit_failures_total[5m])
```

### Partitions assigned (detect rebalance storms)
```promql
kafka_consumer_partitions_assigned
```

### Rebalance rate (alert if > 2/hour)
```promql
rate(kafka_consumer_rebalances_total[1h]) * 3600
```

### Stuck partition detection (offset not advancing)
```promql
changes(kafka_consumer_last_committed_offset[5m]) == 0
and on(partition)
rate(kafka_consumer_messages_polled_total[5m]) > 0
```
